...
func react(t *testing.T, config string, trigger json.RawMessage) []engine.Emit {
	t.Helper()
	r, err := gate.Factory(engine.Subscriber{Name: "g", BodyConfig: json.RawMessage(config)})
	if err != nil {
...
func TestGate_RequiresEmitAndWhen(t *testing.T) {
	if _, err := gate.Factory(engine.Subscriber{Name: "g", BodyConfig: json.RawMessage(`{"when":["done"]}`)}); err == nil {
		t.Error("Factory with no emit returned nil, want error")
	}
	if _, err := gate.Factory(engine.Subscriber{Name: "g", BodyConfig: json.RawMessage(`{"emit":"x"}`)}); err == nil {
