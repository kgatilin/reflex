...
func react(t *testing.T, config string, trigger json.RawMessage) []engine.Emit {
	t.Helper()
	r, err := entry.Factory(engine.Subscriber{Name: "e", BodyConfig: json.RawMessage(config)})
	if err != nil {
...
func TestEntry_RequiresEmitKind(t *testing.T) {
	if _, err := entry.Factory(engine.Subscriber{Name: "e", BodyConfig: json.RawMessage(`{}`)}); err == nil {
