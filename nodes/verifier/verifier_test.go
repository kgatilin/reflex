...
func react(t *testing.T, config string) []engine.Emit {
	t.Helper()
	r, err := verifier.Factory(engine.Subscriber{Name: "v", BodyConfig: json.RawMessage(config)})
	if err != nil {
...
func TestVerifier_RequiresCommand(t *testing.T) {
	if _, err := verifier.Factory(engine.Subscriber{Name: "v", BodyConfig: json.RawMessage(`{}`)}); err == nil {
