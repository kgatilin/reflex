...
| **2a — serializable body descriptors + factory registry** (doc 29 Iteration 2): `Node.BodyKind`+`BodyConfig` ride on `sys.node.registered`; `engine.BodyResolver` + `WithBodyResolver`; `engine.Load(log)` rebuilds bodies from the log; `nodes.Register`/`Resolver`; `llm.Factory`/`Declare` | `engine/resolver.go` + `nodes/registry.go` + `nodes/llm` | **done, verified** (`79fb036`); a descriptor topology recovers from the log alone and runs; unknown-kind/no-resolver rejected (`-race`) |
