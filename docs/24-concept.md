...
### 4. The engine: two mechanisms, one consulted projection

([16](./16-engine-architecture.md), [17](./17-quiescence-prior-art.md))

```
append     put an event on the log (the sole write)
dispatch   fan out to the live table — stamping correlation, consulting
           the progress projection, enforcing budgets
```

Everything else is derived from the **progress projection** — a fold of
causal structure only (the engine never branches on payloads):

- **Quiescence is obligation counting** (Naiad's occurrence counts):
  dispatch to N subscribers opens N obligations; a handler completing
  closes one; zero ⇔ quiescent. An orphan is the zero-crossing of an
  event's own counter — not a post-hoc sweep. A pending plugin call
  holds its obligation until the **result event is appended**. Counters
  are maintained per scope root on the ancestor chain — O(nesting
  depth) per event, no DAG traversal on the hot path.
- **Scopes are cut points in the DAG**: root event + dominated cone,
  with the **sealing rule** (`scope.closed` is the boundary through
  which causality exits — the consumer of a closure lives in the parent
  scope by geometry) and the **partition rule** (an event belongs to its
  nearest enclosing scope; nesting is transitive quiescence for free).
- **Rooting has exactly two sources, no fan-out heuristic**: declared
  (`scopes:` block — root kind, budget, deadline, `closed_when`) and
  node-rooted (a firing that emits work roots an instance; the name
  defaults to the node — `scope.brain.closed`). Every scope is named;
  instances are distinguished causally, never by name.
- **The closure algebra** is the sync vocabulary, none of it a node
  type: `quiescent` (default, failure-tolerant), `quiescent OR any
  *.failed` (errgroup, opt-in), first-terminal (race), `count ≥ N`
  (quorum). Every predicate that can close early **must** cancel the
  remaining cone. Predicates read structure, never payload values.
- **Cancellation**: two sources only — deadline and explicit cancel
  (never handler errors). Propagated as refused dispatch, confined to
  exactly one cone; in-flight tools finish, their results are appended
  but not dispatched.
- **Budgets, and loops are budgets**: soft warning
  (`scope.budget_low`) into the cone one step ahead of the hard
  backstop (`scope.budget_exhausted`). `loop: max_iterations` dissolves
  into a per-kind budget on the scope; a **mandatory default budget on
  the session scope** makes all work bounded; the Tarjan cycle check is
  demoted to a lint.
...
### 5. Projections: declared folds over causal horizons
...
### 6. Topology management: changesets
...
### 7. Standing conventions and patterns
...
### 8. The coding agent and the bootstrap
...
### 9. Implementation status and supersession
...
4. **`sys.node.updated` does not exist** — config (including the
   prompt) changes today are deregister + register. Prompt engineering
   through changesets deserves a first-class update kind: the log then
   versions prompts with "which change moved the metric" attached.
